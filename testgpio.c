...
int set_ledport(unsigned short port ,unsigned char state);
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "hage:p:s:")) != -1) {
     switch (opt) {
     	case 'h':
...
int set_all_port(unsigned char state)
{
	set_port(GPIO_PIN_1,state);
	set_port(GPIO_PIN_3,state);
	set_port(GPIO_PIN_5,state);
	set_port(GPIO_PIN_7,state);
}
...
int set_ledport(unsigned short port ,unsigned char state)
{
	...
	int setv = state;
	if(setv){
		retval = ioctl(fd,SET_LED_HIGHLEVEL,&port_index);
		printf("set port %d value is %d\n",port+1,setv);
		if(retval < 0){
			printf("Set gpio %d high level failed, err:%s\n",port_index+1,strerror(errno));
			return -1;
		...
		retval = ioctl(fd,SET_LED_LOWLEVEL,&port_index);
		printf("set port %d value is %d\n",port+1,setv);
		if(retval < 0){
			printf("Set gpio %d low level failed, err:%s\n",port_index+1,strerror(errno));
			return -1;
...
void poll_all_port_status()
{
	unsigned char pre_port_status[]={0,1,0,1,0,1,0,1};
